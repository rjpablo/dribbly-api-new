namespace DribblyAPI.Migrations
{
    ...
    public partial class MadeTeamCityIdNullable : DbMigration
    {
        public override void Up()
        {
            AlterColumn("dbo.Teams", "cityId", c => c.Int());
            ...
            string str = @"ALTER VIEW FullDetailedTeams
                ...
                FROM            dbo.Teams AS a LEFT JOIN
                             (SELECT        COUNT(*) AS gameCount, teamId
                               FROM            dbo.GameTeams
                               GROUP BY teamId) AS b ON a.teamId = b.teamId LEFT JOIN
                             (SELECT        COUNT(*) AS winCount, winningTeamId
                               FROM            dbo.Games
                               GROUP BY winningTeamId) AS c ON a.teamId = c.winningTeamId LEFT JOIN
            ...
            string str = @"ALTER VIEW FullDetailedTeams
                ...
                FROM            dbo.Teams AS a INNER JOIN
                             ...
                             (SELECT        COUNT(*) AS lossCount, d.teamId
                               ...
                               GROUP BY d.teamId) AS d_1 ON a.teamId = d_1.teamId";
            Sql(str);
            AlterColumn("dbo.Teams", "cityId", c => c.Int(nullable: false));
        }
    }
}
